...
|`WARNING: it appears this device does not use an ESP82xx and therefore cannot install ESP based firmware` in web log|Device uses a non-ESP chip (ECR6600, BK7231, RTL8710, W800, etc.)|**Alternative firmware available!** See [Alternative Chips and Flashing Methods](Alternative-Chips-And-Flashing) for serial/UART flashing with OpenBeken firmware. Devices with ECR6600, BK7231, and other chips can be flashed - just not via OTA. Related: [Issue #1157](https://github.com/ct-Open-Source/tuya-convert/issues/1157)|
